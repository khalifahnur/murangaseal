...
const bottomLinks = [
  ...
  { id: 3, txt: "MEN'S TEAM", link: "/#mens-team", sectionId: "mens-team" },
  { id: 4, txt: "SHOP", link: "/#shop", sectionId: "shop" },
  { id: 5, txt: "CLUB", link: "/#club", sectionId: "club" },
];
...
export default function Header() {
  ...
  const handleScroll = useCallback(() => {
    ...
    if (Math.abs(currentScrollY - lastScrollY) > 5) {
      ...
      if (scrollDelta > 2 && currentScrollY > 100) {
  ...
  const updateActiveSection = useCallback((scrollY: number) => {
    const sections = bottomLinks.map(link => link.sectionId);
    const buffer = 100;
  ...
  const handleDesktopLinkClick = (e: React.MouseEvent, link: typeof bottomLinks[0]) => {
    e.preventDefault();
    if (link.sectionId) {
      ...
      <div className={`bg-white shadow-md transition-all duration-300 ${
        ...
        <div className="px-4 py-3 flex items-center justify-between">
            ...
            {bottomLinks.map((item) => (
              ...
              >
                ...
                <div className={`absolute bottom-0 left-0 h-0.5 bg-primary transition-all duration-300 ease-out ${
                  activeSection === item.sectionId || item.txt === "CLUB"
                    ? 'w-full' 
          ...
          >
            ...
            <span className={`w-6 h-0.5 bg-black transition-all duration-300 mt-1.5 ${
        ...
        {/* Mobile Menu Overlay */}
        <div className={`lg:hidden fixed inset-0 bg-black/50 backdrop-blur-sm transition-all duration-300 z-50 ${
          isMobileMenuOpen 
        ...
        <div className={`lg:hidden fixed top-0 right-0 h-full w-80 bg-white shadow-2xl transition-transform duration-500 ease-in-out z-50 ${
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="p-6 border-t border-gray-200">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-sm font-semibold text-black">In partnership with</p>
